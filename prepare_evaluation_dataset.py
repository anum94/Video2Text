...
if __name__ == '__main__':
    ...
    file_name = f"evaluation_samples/{ds}/instructions.txt"

    # Define the instructions
    instructions = [
        f"In the folder you will find {SAMPLES_PER_MODEL} folders. Each folder represents one game."
        f" Inside each folder you will find a video clip from a game and {len(model_dict.keys())} subfolders, each containg the commentaries generated by a model."
        f"Each model generated commentary using 2 methods, so in total you would have {len(model_dict.keys()) *2} commentaries"
        "For each sample, your task is to watch the video clip by overlaying the subtitles and assigning a score to each commentary in the provided excel file.",
        "KEI [0-5]: Key Identification of Events -> If the commentary  was able to identify the transition/changes in the game",
        "Pause-awareness [0-5]:" "We are trying to teach Patience to our model. Did the model stay quite when nothing was happening? or did it continue to generate unnecessary commentary. Assign higher score if it was patient",
        "Coherence [0-5]: Was the generated commentary coherent?",
        "Naturalness [0-5]: Did the generated commentary seem natural?",

    ]

    # Write the instructions to the text file
    with open(file_name, "w") as file:
        for line in instructions:
            file.write(line + "\n")

    print(f"Instructions written to {file_name}")
