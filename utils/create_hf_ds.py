...
def create_ds(folder):
    ...
    all_game_path = [
        os.path.join(video_directory, name)
        for name in os.listdir(video_directory)
        if os.path.isdir(os.path.join(video_directory, name))
    ]
    ...
    for i, game_path in enumerate(all_game_path):
        ...
        if transcription_file is not None:
            mp4_file = [
                ...
                and os.path.isfile(os.path.join(game_path, file))
                and "客観" in file
            ][0]
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--transcriptions_dir",
        required=False,
        default="transcriptions_whole_data_english",
        type=str,
        help="Directory containing the videos "
        "and respective commentary in recordings and transcriptions_whole_data_english folder",
    )
