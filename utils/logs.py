...
def write_to_wb(
    ...
    feedback_output: tuple,
    icl_output: tuple,
    realtime_output: tuple,
    config,
    WB=True,
):
    if WB:
    ...
    if WB:
        ...
        axs[0, 1].plot(
            x, ref_timing, marker="o", label="ref_timing", linestyle="-", color="g"
        )
        axs[0, 1].legend()
        ...
        ref_gen_words = np.array([len(gen.split()) for gen in ref_generations]).astype(
            int
        )
        ...
        axs[1, 1].plot(
            x, i_gen_words, marker="o", label="icl_gen_words", linestyle="-", color="y"
        )
        axs[1, 1].legend()
