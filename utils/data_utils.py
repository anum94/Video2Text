...
def read_srt(input_file_path):
    ...
    try:
        ...
        sub = pysrt.SubRipItem(
            1, start="00:00:04,000", end="00:02:08,000", text="Hello World!"
        )
        subs.append(sub)
    return subs
...
def compute_10_percent(ref_list, pred_list, n_intervals=10):
    ...
    for i, (start, end) in enumerate(intervals):
        ...
        BLEUscore = nltk.translate.bleu_score.sentence_bleu(
            [[ref for ref in ref.split()]],
            [pred for pred in hyp],
...
def compute_metrics(
    ...
    p_corr, _ = pearsonr(
        [1 if i == True else 0 for i in ref_timing],
    ...
    smoothie = SmoothingFunction().method4
    BLEUscore = nltk.translate.bleu_score.sentence_bleu(
        [[ref for ref in ref_commentary.split()]],
        [pred for pred in pred_commentary],
        smoothing_function=smoothie,
    )
    ...
    res = {
        "correlation": (correlations.count(1)) / len(correlations),
        "ROUGE_L": rouge_L,
        "BLEU": BLEUscore,
        "ref_timing": list(ref_timing),
        "pearson": p_corr,
        "pred_timing": list(pred_timing),
        "bins": metrics_over_intervals,
        "BERTScore": bert_F1,
        "LAAL": laal,
        "LA": la,
    }
...
def convert_text_to_srt(
    file_path: str = None,
    talking_speed_sample: str = "../RaceCommentary/transcriptions_whole_data_english/AC_120221-180622_R_ks_audi_r8_plus_ks_nurburgring_layout_sprint_a_kyakkan.merged.mp4_translated.srt",
):
            ...
            if match:
                ...
                if ut and "WAIT" not in ut:
                    ...
                    end_time = seconds_to_timestamp(
                        t + (num_of_words * seconds_per_word)
                    )
